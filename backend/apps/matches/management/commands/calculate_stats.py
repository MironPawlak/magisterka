...
from apps.matches.models import Match, Champion, Statistic, MatchChampion, SimpleMatch
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for champion in champions:
            win = MatchChampion.objects.filter(champion=champion).filter(win=True).count()
            lose = MatchChampion.objects.filter(champion=champion).filter(win=False).count()
